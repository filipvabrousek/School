...
if ($sekce == "formular") {
    ...
    if (isset($_REQUEST["odeslat"])) { //prisla data z formulare? Byl odeslan?
        ...
        $characters = "qwertzuioplkjhgfdsayxcvbnměščřžýáíéQWERTZUIOPLKJHGFDSAMNBVCXYĚŠČŘŽÝÁÍÉäöüß";
            ...
            if (strlen($name) > 2) {
                ...
                for ($i = 0; $i<strlen($name); $i++){
                   // print(($i + 1).". ".$name[$i]."<br>");
                    ...
                    if (strpos($charactesr, $name[i]) == false) || (strpos($characters, $name[0]))
                    $testing = strtoupper($name);
                  //  echo $testing;
...
<br>
    ...
    <input type="checkbox" name = "product" value="iPhone XS Max" <?php if (isset($product)) {$product = $_REQUEST["product"]; if($product == "iPhoneXS Max"){print "checked = \"checked\"";}} /*else {print("checked");*}*/?>>
     <br>
        
        <label for = "radio" name = "radio">iPhone Xr</label>
    <input type="checkbox" name = "product" value="iPhone Xr" <?php if (isset($product)) {$product = $_REQUEST["product"]; if($product == "iPhoneXr"){print "checked = \"checked\"";}} /*else {print("checked");}*/?>>
